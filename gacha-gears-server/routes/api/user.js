  ...
  if (user.length > 0) {
    try {
      if (await argon2.verify(user[0].password, req.body.password)) {
        ...
        res.cookie('token', jwt, { httpOnly: true, maxAge: 7200000, sameSite: 'none', secure: true });
        res.cookie('public-token', user[0].username, { maxAge: 7200000, sameSite: 'none', secure: true });
        res.send("token sent");
      }
      else {
        res.status(401).send();
